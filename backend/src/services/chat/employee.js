    ...
    // If client does not accept SSE reliably, fall back to JSON chunking
    setupSSEHeaders(res);
    ...
    const agent = getAgentForSession(Number(sessionId));
        ...
        if (agentResult && agentResult.message) {
          ...
          // Send job data first if available (so cards appear quickly)
        ...
        // Handle EPIPE errors gracefully
        if (err.code === 'EPIPE') {
          console.log('Client disconnected during streaming, ignoring EPIPE error');
          return;
        }
