...
export default {
  ...
  methods: {
    async getUserIdFromEmail() {
      // Use employerToken first, fallback to authToken for compatibility
      const token = localStorage.getItem("employerToken") || localStorage.getItem("authToken");
      if (!token) return;
    ...
    async fetchProfile() {
      try {
        ...
        if (res.ok) {
          ...
          this.job_provider_id = data.job_provider_id || "";
        }
      } catch (err) {
    ...
    async updatePaymentStatus() {
      try {
        // Use employerToken first, fallback to authToken for compatibility
        const token = localStorage.getItem("employerToken") || localStorage.getItem("authToken");
        const res = await fetch(`${globalVariable}/admin/approve/${this.users_id}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        const data = await res.json();
        if (res.ok) {
          console.log("Backend approval successful:", data.message);
          this.payFeedback = "Payment approved! Access granted. ✓";
          this.payFeedbackColor = "green";
          setTimeout(() => {
            this.closePayment();
            location.reload();
          ...
          console.error("Backend approval failed:", data.message);
          this.payFeedback = "Payment succeeded, but approval failed. Contact support.";
          this.payFeedbackColor = "red";
        }
      } catch (err) {
        console.error("Error updating payment status on backend:", err);
        this.payFeedback = "Payment succeeded, but couldn't update status. Contact support.";
        this.payFeedbackColor = "red";
      }
    },
  },
